...
int second_chance_fifo()
{
  ...
  for (i = 0; (i < MAX_PYSC_PAGES) && (index == -1); i++)
  {
    ...
    pde_t *pte = walkpgdir(curproc->pgdir, (char *)temp_v_addr, 0);
    if (*pte & PTE_A)
    {
      *pte &= ~PTE_A;
...
void update_pages_age_counter()
{
  ...
  for (i = 0; i < MAX_PYSC_PAGES; i++)
  {
    if (curproc->ram_arr[i].occupied)
    {
      ...
      pte_t *pte = walkpgdir(curproc->pgdir, (void *)curproc->ram_arr[i].virtual_adrr, 0);
      if (*pte & PTE_A)
      {
...
void handle_page_fault()
{
  ...
  if (p->pid > 2 && !is_none_paging_policy())
  {
    ...
    {
      struct page exile_ram = p->ram_arr[find_ram_by_policy()]; //this will change in task 3
      disk_to_ram(start_pfault_va, new_physical_adrr);
